...
# Default threshold: when a connected component has more VNets than this,
# it is automatically split into an overview + detail sub-diagrams.
DEFAULT_MAX_VNETS_PER_DIAGRAM = 15


class DiagramDetail(Enum):
    """Controls how much detail is rendered in each diagram."""

    summary = "summary"    # VNets as boxes with address spaces only — no subnets/NSGs/RTs/DNS
    standard = "standard"  # VNets with subnets, peerings, gateways (default)
    full = "full"          # Everything including NSGs, route tables, DNS zones
...
def generate_mermaid_diagrams(
    ...
    *,
    ...
    When a connected component exceeds *max_vnets*, an overview diagram at
    ``summary`` detail is emitted first, followed by chunked detail diagrams
    each containing at most *max_vnets* VNets.
    """
    ...
    for _root, members in sorted(groups.items(), key=lambda kv: (-len(kv[1]), kv[0])):
        ...
        needs_chunking = len(members) > max_vnets and not is_standalone

        if needs_chunking:
            # --- Overview diagram at summary level ---
            overview_title = "Overview: " + " ↔ ".join(sorted(members)[:6])
            if len(members) > 6:
                overview_title += f" (+{len(members) - 6} more)"
            diagrams.append((
                overview_title,
                _build_diagram(
                    members, vnet_map, topology, member_set=set(members),
                    subnet_to_vnet=subnet_to_vnet,
                    pip_by_id=pip_by_id, pip_by_resource=pip_by_resource,
                    detail=DiagramDetail.summary, node_id=node_id,
                ),
            ))

            # --- Chunked detail diagrams ---
            # Identify hub VNets (most peerings) to anchor each chunk
            peering_counts = {
                name: len(vnet_map[name].peerings) for name in members
            }
            sorted_by_peerings = sorted(
                members, key=lambda n: peering_counts[n], reverse=True,
            )
            # The top hub VNet(s) appear in every chunk for context
            hub_names = [
                n for n in sorted_by_peerings
                if peering_counts[n] > max_vnets // 2
            ][:2]
            hub_set = set(hub_names)

            remaining = [n for n in sorted(members) if n not in hub_set]
            chunk_size = max(1, max_vnets - len(hub_set))
            for i in range(0, len(remaining), chunk_size):
                chunk_members = hub_names + remaining[i : i + chunk_size]
                chunk_num = i // chunk_size + 1
                total_chunks = (len(remaining) + chunk_size - 1) // chunk_size
                chunk_title = (
                    f"Detail {chunk_num}/{total_chunks}: "
                    + " ↔ ".join(sorted(chunk_members)[:4])
                )
                if len(chunk_members) > 4:
                    chunk_title += f" (+{len(chunk_members) - 4} more)"

                diagrams.append((
                    chunk_title,
                    _build_diagram(
                        chunk_members, vnet_map, topology,
                        member_set=set(chunk_members),
                        subnet_to_vnet=subnet_to_vnet,
                        pip_by_id=pip_by_id, pip_by_resource=pip_by_resource,
...
def _build_diagram(
    ...
    *,
    ...
    """Build the Mermaid source for a single diagram."""
    lines = ["graph LR"]
    ...
    for vname in sorted(members):
        ...
        if detail != DiagramDetail.summary:
            for subnet in vnet.subnets:
    ...
    for vname in members:
        ...
        for peering in vnet.peerings:
            remote = peering.remote_vnet_name
            # Only draw peering if the remote VNet is also in this diagram
            if remote not in member_set:
                continue
            remote_vid = node_id(remote)
    ...
    # --- summary diagrams stop here ---
    if detail == DiagramDetail.summary:
        lines.append("")
        lines.append("    classDef vnet fill:#e1f5fe,stroke:#0288d1")
        return "\n".join(lines)

    # NSG associations (standard + full)
    if detail == DiagramDetail.full:
        for vname in members:
            vnet = vnet_map[vname]
            for subnet in vnet.subnets:
                if subnet.nsg_id:
    ...
    # Route Table associations (full only)
    if detail == DiagramDetail.full:
        for vname in members:
            vnet = vnet_map[vname]
            for subnet in vnet.subnets:
    ...
    # Private DNS Zones linked to VNets in this group (full only)
    if detail == DiagramDetail.full:
